...
BOOST_FIXTURE_TEST_CASE(improves_feerate, TestChain100Setup)
{
    ...
    changeset->RemoveTx(entry1);
    changeset->RemoveTx(entry2);
    changeset->AddTx(tx1_conflict, tx1_fee, 0, 1, 0, false, 4, LockPoints());
    changeset->AddTx(tx3, tx2_fee, 0, 1, 0, false, 4, LockPoints());
    ...
    changeset->RemoveTx(entry1);
    changeset->RemoveTx(entry2);
    changeset->AddTx(tx1_conflict, tx1_fee+1, 0, 1, 0, false, 4, LockPoints());
    changeset->AddTx(tx3, tx2_fee, 0, 1, 0, false, 4, LockPoints());
    ...
    changeset->RemoveTx(entry2);
    changeset->AddTx(tx1_conflict, tx1_fee, 0, 1, 0, false, 4, LockPoints());
    changeset->AddTx(entry4.GetSharedTx(), tx2_fee, 0, 1, 0, false, 4, LockPoints());
    BOOST_CHECK(ImprovesFeerateDiagram(*changeset) == std::nullopt);
    changeset.reset();
    ...
    changeset->RemoveTx(entry1);
    changeset->RemoveTx(entry2);
    changeset->RemoveTx(entry5);
    changeset->AddTx(tx1_conflict, tx1_fee, 0, 1, 0, false, 4, LockPoints());
    changeset->AddTx(entry4.GetSharedTx(), tx2_fee + entry5->GetModifiedFee() + 1, 0, 1, 0, false, 4, LockPoints());
...
BOOST_FIXTURE_TEST_CASE(calc_feerate_diagram_rbf, TestChain100Setup)
{
    ...
    {
        auto changeset = pool.GetChangeSet();
        changeset->RemoveTx(entry_low);
        changeset->AddTx(replacement_tx, 0, 0, 1, 0, false, 4, LockPoints());
    ...
    // Non-zero replacement fee/size
    {
        auto changeset = pool.GetChangeSet();
        changeset->RemoveTx(entry_low);
        changeset->AddTx(replacement_tx, high_fee, 0, 1, 0, false, 4, LockPoints());
...

    {
        ...
        changeset->RemoveTx(entry_low);
        changeset->RemoveTx(entry_high);
        changeset->RemoveTx(entry_normal);
        changeset->AddTx(replacement_tx, high_fee, 0, 1, 0, false, 4, LockPoints());
    ...
    {
        auto changeset = pool.GetChangeSet();
        changeset->RemoveTx(entry_high_2);
        changeset->RemoveTx(entry_low_2);
        changeset->AddTx(replacement_tx, high_fee, 0, 1, 0, false, 4, LockPoints());
    ...
    {
        auto changeset = pool.GetChangeSet();
        changeset->RemoveTx(conflict_1_entry);
        changeset->RemoveTx(conflict_2_entry);
        changeset->RemoveTx(conflict_3_entry);
        changeset->AddTx(replacement_tx, high_fee, 0, 1, 0, false, 4, LockPoints());
...

    {
        ...
        changeset->RemoveTx(conflict_2_entry);
        changeset->RemoveTx(conflict_3_entry);
        changeset->RemoveTx(conflict_1_child_entry);
        changeset->AddTx(replacement_tx, high_fee, 0, 1, 0, false, 4, LockPoints());
...

    {
        ...
        changeset->RemoveTx(conflict_3_entry);
        changeset->RemoveTx(conflict_1_child_entry);
        changeset->RemoveTx(conflict_1_grand_child_entry);
        changeset->AddTx(replacement_tx, high_fee, 0, 1, 0, false, 4, LockPoints());
