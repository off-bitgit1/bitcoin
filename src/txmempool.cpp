...
{
    AssertLockHeld(cs);
    RemoveStaged(changeset->m_all_conflicts, false, MemPoolRemovalReason::REPLACED);
    ...
    for (size_t i=0; i<changeset->m_entry_vec.size(); ++i) {
        ...
        auto node_handle = changeset->m_stage_tx.extract(tx_entry);
...
util::Result<std::pair<std::vector<FeeFrac>, std::vector<FeeFrac>>> CTxMemPool::CTxMemPoolChangeSet::CalculateChunksForRBF()
{
    ...
    auto err_string{m_pool->CheckConflictTopology(m_all_conflicts)};
    ...
    for (auto txiter : m_all_conflicts) {
    ...
    for (auto direct_conflict : m_all_conflicts) {
        ...
        if (direct_conflict->GetMemPoolParentsConst().size() > 0) {
            ...
            if (!m_all_conflicts.contains(m_pool->mapTx.iterator_to(parent))) {
...
CTxMemPool::CTxMemPoolChangeSet::TxHandle CTxMemPool::CTxMemPoolChangeSet::AddTx(const CTransactionRef& tx, const CAmount fee, int64_t time, unsigned int entry_height, uint64_t entry_sequence, bool spends_coinbase, int64_t sigops_cost, LockPoints lp)
{
    ...
    if (delta) m_stage_tx.modify(newit, [&delta](CTxMemPoolEntry& e) { e.UpdateModifiedFee(delta); });

    feerate += FeeFrac(newit->GetModifiedFee(), newit->GetTxSize());
...
std::vector<TxEntry::TxEntryRef> CTxMemPool::CTxMemPoolChangeSet::CalculateParentsOf(const CTransactionRef& tx)
{
    ...
    for (const uint256 &hash : inputs) {
        ...
        if (piter) {
            ...
            auto it = m_stage_tx.find(hash);
            if (it != m_stage_tx.end()) {
                ret.emplace_back(*it);
            }
        }
    }
